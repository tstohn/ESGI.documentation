...
Application example for multimodal data using SIGNALseq, containing reads for both transcripts and protein levels, generated by combinatorial indexing. Here, there are separate FASTQ files for both the transcripts and the proteins, both containing forward and reverse reads. Below instructions on how to download the data. 
